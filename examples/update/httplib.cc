...
template <typename T>
bool process_server_socket_core(socket_t sock,
...
template <typename T>
bool process_server_socket(socket_t sock, size_t keep_alive_max_count,
...
uint64_t get_header_value_uint64(const Headers &headers, const char *key,
                                        uint64_t def = 0) {
...
Server::Server() : svr_sock_(INVALID_SOCKET), is_running_(false) {
#ifndef _WIN32
...
bool Client::process_socket(Socket &socket,
...
void Client::set_keep_alive(bool on) { keep_alive_ = on; }

void Client::set_follow_location(bool on) { follow_location_ = on; }

void Client::set_tcp_nodelay(bool on) { tcp_nodelay_ = on; }
...
void Client::set_decompress(bool on) { decompress_ = on; }
...
void SSLClient::enable_server_certificate_verification(bool enabled) {
